...
*** variables ***
${file}    ${resource_dir}/tests/pod_soft_antiaffinity.yaml
${dest_file}    /root/pod_soft_antiaffinity.yaml
...
Test Pod Soft Antiaffinity with Sufficient Nodes 
    Create k8s App    ${dest_file}    ${ns}
    Log    Getting Pod Names
...
Test Pod Soft Antiaffinity with InSufficient Nodes 
    Create k8s App    ${dest_file}    ${ns}
    Log    Verifying pod status    console=yes   
    @{pod-list}=    k8s_resources.Get pod names
    Set Suite Variable        @{pod-list}
